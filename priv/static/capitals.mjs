...
function append(first3, second) {
  return do_append(reverse(first3), second);
}
...
var Model = class extends CustomType {
  constructor(score, correct, incorrect, current_country, current_guess) {
    super();
    this.score = score;
...
var UserUpdatedGuess = class extends CustomType {
  constructor(value3) {
    super();
    this.value = value3;
  }
...
function next_country(guessed) {
  let remaining_countries = (() => {
    ...
    return filter(
      _pipe,
      (country) => {
        return contains(guessed, country) === false;
      }
    );
  })();
  let $ = first(remaining_countries);
  if ($.isOk()) {
    let country = $[0];
    return country;
  } else {
